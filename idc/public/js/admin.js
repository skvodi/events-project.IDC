    ...
    return `
        <div class="event ${isFinished ? "event-finished" : ""}">
            ...
            <div class="btn-row">
                ...
                <button class="btn-warning" onclick="toggleEditForm(${event.id})">Редактировать</button>
            ...
            <div class="edit-form" id="edit-${event.id}">
                <input type="text" name="name" value="${escapeHtml(event.name)}" placeholder="Название" required>
                <textarea name="description" placeholder="Описание" rows="3">${escapeHtml(event.description || "")}</textarea>
                <select name="city_id">
                    <option value="">Выберите город</option>
                    ${cities.map(c => `<option value="${c.id}" ${c.id == event.city_id ? "selected" : ""}>${escapeHtml(c.name)}</option>`).join("")}
                </select>
                <input type="text" name="address" value="${escapeHtml(event.address || "")}" placeholder="Адрес">
                <select name="category_id">
                    <option value="">Выберите категорию</option>
                    ${categories.map(c => `<option value="${c.id}" ${c.id == event.category_id ? "selected" : ""}>${escapeHtml(c.name)}</option>`).join("")}
                </select>
                <input type="datetime-local" name="event_date" value="${escapeHtml(event.event_date || "")}">
                <input type="datetime-local" name="registration_end" value="${escapeHtml(event.registration_end || "")}">
                <select name="option_ids" multiple size="4">
                    ${eventOptions.map(option => `<option value="${option.id}" ${selectedIds.includes(String(option.id)) ? "selected" : ""}>${escapeHtml(option.name)} (${escapeHtml(option.type_name)})</option>`).join("")}
                </select>
                ...
                <input type="number" name="max_participants" value="${escapeHtml(event.max_participants || "")}" placeholder="Макс. участников" min="${event.participants_count || 0}">
                <div class="btn-row">
                    <button type="button" onclick="saveEvent(${event.id})">Сохранить</button>
                    <button type="button" class="btn-danger" onclick="toggleEditForm(${event.id})">Отмена</button>
                </div>
            </div>
        ...
        .then(participants => {
            ...
            list.innerHTML = participants.map(p => `
                <div class="participant-row">
                    <span>${escapeHtml(p.full_name)} (${escapeHtml(p.email)})</span>
